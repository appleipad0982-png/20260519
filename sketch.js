...
// ── 修改:選單手勢狀態 ────────────────────────────────────────
let menuHoldT = null;             // 選單手勢計時器
let menuPose = null;              // 目前鎖定的選單手勢 ('paper' or 'rock')
const MENU_HOLD = 1200;           // 選單手勢需維持的時間 (ms),用較長時間避免誤觸發
...
// ─────────────────────────────────────────────────────────────
//  GESTURE CLASSIFICATION (移除原本的 thumbs_up,改用 rock/paper 做選單)
// ─────────────────────────────────────────────────────────────
function classify(l) {
...
function dIdle() {
    ...
    if (!lm) {
        ...
        const isValid = PICKS.includes(stable);
        boldT(isValid ? `鎖定中:${EM[stable]} ${LB[stable]}` : `請換個手勢`, W / 2, H - 102, 20, isValid ? '#00FF88' : '#FFD93D', null, isValid ? '#00FF88' : null);
        ...
        smT(!isValid ? '⚠️ 請比出拳手勢' : pct < 1 ? '保持手勢,即將開始...' : 'GO!', W / 2, H - 44, 13, 'rgba(255,255,255,.7)');
    } else {
...
function dMenu() {
    ...
    smT('點擊按鈕,或維持手勢 1.2 秒選擇', W / 2, H / 2 + 6, 14);
...
function update() {
    ...
    // ── 修改:選單狀態改用 ✋ paper / ✊ rock,需維持 1.2 秒 ──
    if (st === 'menu') {
        if (stable === 'paper' || stable === 'rock') {
            ...
            if (now - menuHoldT >= MENU_HOLD) {
                if (menuPose === 'paper') startGame();        // ✋ → 繼續
                else if (menuPose === 'rock') enter('ended'); // ✊ → 結束
                menuHoldT = null;
                menuPose = null;
            }
        } else {
